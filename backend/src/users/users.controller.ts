...
export class UsersController {
  ...
  @ApiResponse({
    status: 404,
    description: 'No available locations in category',
  })
  getRandomLocation(
    @Query('category') category: string,
    @Query('userId') userId: string,
  ) {
    const location = this.usersService.getRandomLocation(
      category as any,
      userId,
    );
    if (!location) {
      throw new NotFoundException(
        `No available locations in category: ${category}`,
      );
    }
    return location;
  }
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid QR code or already completed',
  })
